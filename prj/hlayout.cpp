...
HLayout::HLayout(QObject *parent) : QObject(parent)
{
...
HLayoutCell HLayout::merge(int lt, int rb){
    ...
    if (getLayoutCell(lt, cell_lt) && getLayoutCell(rb, cell_rb)){
        int r1 = qMin(cell_lt.r1, cell_rb.r1);
        int r2 = qMax(cell_lt.r2, cell_rb.r2);
        int c1 = qMin(cell_lt.c1, cell_rb.c1);
        int c2 = qMax(cell_lt.c2, cell_rb.c2);

        qDebug("r:%d~%d c:%d~%d", r1,r2,c1,c2);
